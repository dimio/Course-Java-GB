...
class GameEngine {
  ...
  private char realPlayerSign, computerPlayerSign;
  private char[][] map;
  ...
  GameEngine(GameBoard board) {
    this.board = board;
    this.settings = board.getGameSettings();
    this.map = board.getGameField();
  ...
  boolean getHasWinTurn() {
    return hasWinTurn;
  }

  TurnCoord makeIsFirstTurn() {
  ...
  TurnCoord makeStupidTurn() {
      ...
      for (int j = 0; j < settings.getBoardWidth(); j++){
        ...
        if (board.isTurnable(i, j)){
          ...
          if (turnSizeC >= settings.getWiningStreakSize()){
            hasWinTurn = true; //устанавливать поле через приватный сеттер или так?
    ...
    if (hasWinTurn || turnWinC[0] > turnWinH[0]){
      turnCoord = new TurnCoord(turnWinC[1], turnWinC[2]); //i,j
    }
    // если у соперника лучше - попытаться сорвать его серию
    else {
      turnCoord = new TurnCoord(turnWinH[1], turnWinH[2]); //i,j
    }

    return turnCoord;
  }

  int inCellMaxStreakSize(int i, int j) {
    /*
